...
post_pattern = re.compile(r'\b(officer|chairman|president|leader|manager|associate|analyst|engineer|accountant|assistant|lead|programm?er|designer|developer|intern|consultant|chief|administrator|trainer|trainee|executive|writer|supervisor|faculty|architect|scientist|researcher|doctor|investigator|inspector|auditor|lecturer|teacher|examiner|professor|principal|secretary|assistant|reviewer|librarian|technician|advocate|lawyer|artist|specialist|generalist|operator|tester|(a[\.]?)?g[\.]?m[\.]?|v[\.]?p[\.]?|m[\.]?d[\.]?|C[\.]?[EFOT][\.]?O[\.]?|assessor|evaluator|clerk|statistician|mathematician|head|worker|spoc)\b',re.IGNORECASE)
...
def do_instances_match(i1, i2):
	...
	if(i1.startswith("chief executive") and i2 == "ceo"):
		return True
		
	if(i2.startswith("chief executive") and i1 == "ceo"):
		return True
		
	i1_words = i1.split(' ')
	i2_words = i2.split(' ')
		
	if(i1_words[0] == 'mr.' or i1_words[0] == 'ms.'):
		if(len(i1_words) > 1 and i2.endswith(i1_words[-1])):
			return True
		
	if(i2_words[0] == 'mr.' or i2_words[0] == 'ms.'):
		if(len(i2_words) > 1 and i1.endswith(i2_words[-1])):
			return True
			
	i1_match = post_pattern.search(i1)
	i2_match = post_pattern.search(i2)
	if(not(i1_match) and not(i2_match)):
		if(i1.endswith(i2) or i2.endswith(i1)):
			return True
